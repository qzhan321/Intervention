...
suppressPackageStartupMessages({
  ...
  library(vegan)
  library(tidyverse)
  library(ggplot2, lib.loc = "/home/qizhan/R/x86_64-pc-linux-gnu-library/4.0")
  library(cowplot)
  library(scales)
})
run <- 4
...
saveDir0 <- paste0("/project2/pascualmm/QZ/PhD/projects/intervention/files", run, "/actualRuns/PTSAgeDiff2/")
dir.create(saveDir0)
  ...
  for (r in reps) {
    for (m in 1:length(types)) {
      ...
      for (j in 1:length(layers)) {
        ...
        hosts1 <- AgeMOI %>% filter(age <= ageLow)
        if (nrow(hosts1) > 1) {
          hosts1 <- unique(hosts1$host_id)
          print(length(hosts1))
          PTS1 <- PTS[hosts1, hosts1]
          pts1 <- data.frame("PTS" = PTS1[row(PTS1)!=col(PTS1)], "time" = layer, "num" = num, "ageGroup" = "1-10")
          PTSAll <- rbind(PTSAll, pts1)
        }
        
        hosts2 <- AgeMOI %>% filter(age >= ageHigh)
        if (nrow(hosts2) > 1) {
          hosts2 <- unique(hosts2$host_id)
          print(length(hosts2))
          PTS2 <- PTS[hosts2, hosts2]
          pts2 <- data.frame("PTS" = PTS2[row(PTS2)!=col(PTS2)], "time" = layer, "num" = num, "ageGroup" = ">=20")
