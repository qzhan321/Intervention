...
for (i in 1:length(nums)) {
  ...
  for (r in reps) {
    ...
    for (ii in 1:length(layers)) {
      ...
      save(microscopyPosHosts, file = paste0(sampledHostsSaveDir4, "rep_", r, "time_", layer, ".RData"))
      ...
      for (j in 1:length(s_temp)) {
        ...
        if (s >= a & s <= s_single*MOI_max) {
          ...
          df <- data.frame("p" = max(p_c_givens_all), "MOI" = c(1:MOI_max)[which.max(p_c_givens_all)], "numDBLaTypes" = s, 
                           "host_id" = names[j], "num" = num, "rep" = r, "time" = layer)
  ...
  # note that some individuals have their p vector equal to 0, because their number of DBLalpha types falls below 10. Remove them before plotting figures.
}
