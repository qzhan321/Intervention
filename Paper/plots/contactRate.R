  ...
  for (j in 1:length(openness)) {
    ...
    for (k in 1:length(IRSTypes)) {
      ...
      if (IRSType == "10yIRS") {
        ...
        summaryTable2 <- summaryTable1 %>% filter(!(time == preIRS * T_YEAR & IRS %in% c(paste0("I-", 1:14))))
...
part1 <- pAll1[[1]] 
part2 <- pAll2[[1]] + rremove("xlab") + rremove("ylab") 
part3 <- pAll2[[2]] + rremove("xlab") + rremove("ylab")

ggsave(paste0(saveDir3, "IRSContactRate-1.pdf"), part1, width = 14, height = 5.5)
ggsave(paste0(saveDir3, "IRSContactRate-2.pdf"), part2, width = 10, height = 6.25)
ggsave(paste0(saveDir3, "IRSContactRate-3.pdf"), part3, width = 10, height = 6.25)
