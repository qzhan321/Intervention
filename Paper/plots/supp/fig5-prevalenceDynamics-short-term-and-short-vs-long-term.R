...
saveDir0 <- "/project2/pascualmm/QZ/PhD/projects/intervention/natComRevision/plots/figures/supp/fig5/prevalenceDynamicsShortTerm/"
if (!dir.exists(saveDir0)) {
  dir.create(saveDir0)
}
saveDir1 <- "/project2/pascualmm/QZ/PhD/projects/intervention/natComRevision/plots/figures/supp/fig5/prevalenceDynamicsShortTermVsLongTerm/"
...
for (i in 1:length(seasonality)) {
  ...
  for (j in 1:length(openness)) {
    ...
    for (k in 1:length(numsPlot)) {
      ...
      summaryTable4$IRS <- factor(summaryTable4$IRS, levels = c("preIRS", paste0("I-", 1:16)))
      ...
      df1 <- summaryTable4 %>% filter(time == t1) %>% ungroup() %>%
        select(IRS, mP, minP, maxP)
      colnames(df1)[2:4] <- paste0("short_term_rebound_", colnames(df1)[2:4])
      df2 <- summaryTable4 %>% filter(time == t2) %>% ungroup() %>%
        select(IRS, mP, minP, maxP)
      ...
      p1 <- ggplot(df12, aes(x = short_term_rebound_mP, y = longer_term_rebound_mP, col = IRS, fill = IRS))+ 
        geom_point(size = pointSize) +
        geom_errorbarh(aes(xmin = short_term_rebound_minP, xmax = short_term_rebound_maxP), height = pointSize/100, size = pointSize/4) +
        geom_errorbar(aes(ymin = longer_term_rebound_minP, ymax = longer_term_rebound_maxP), width = pointSize/100, size = pointSize/4) +
        ggtitle(label) +
        ...
        theme_bw() + theme(
          ...
          axis.text.x = element_text(color="black", size=sizeV/1.25, angle=0),
      ...
      forwardYs <- 1
      if (s == "seasonal") {
        df1 <- summaryTable4 %>% filter(time == (preIRS + forwardYs)*T_YEAR + 300)
      } else {
        df1 <- summaryTable4 %>% filter(time == (preIRS + forwardYs + 1)*T_YEAR)
      }
      ...
      epiCR
      ggsave(paste0(saveDir6, "num-", min(nums), "-", max(nums), "-cr.pdf"), epiCR, width = 6.45, height = 7.45)
      ggsave(paste0(saveDir7, "num-", min(nums), "-", max(nums), ".pdf"), p1, width = 6.45, height = 7.45)
    }
  }
}
