...
numsPlotList <- list(
  ...
  "seasonal regionally-open" = list(201:209, 219:229, 237:245, 255:265),
...
saveDir0 <- "/project2/pascualmm/QZ/PhD/projects/intervention/natComRevision/plots/figures/supp/fig2/numGenes/"
...
nums_w_reps_list <- list(
  ...
  "seasonal regionally-open" = list(206:209, 226:229, 242:245, 262:265),
  ...
  for (j in 1:length(openness)) {
    ...
    for (k in 1:length(numsPlot)) {
      ...
      for (m in 1:length(nums)) {
        ...
        for (r in reps) {
          ...
          if (length(tsdiff) > 0) {
            ...
            df3App <- data.frame("num" = num, "numGenes" = 0, "time" = tsdiff, "rep" = r, "IRS" = paste0("I-", num - min(nums) + 1))
            df4 <- rbind(df3, df3App)
          } else {
            df4 <- df3
          }
          GenesAll <- rbind(GenesAll, df4)
        }
      }
      ...
      SD2 <- SD %>% group_by(time, IRS, meanCR) %>% 
        summarise(mD=mean(numGenes), maxD=max(numGenes), minD=min(numGenes))
      SD2 <- SD2 %>% mutate(timePlot = time/T_YEAR -preIRS)
      SD2$IRS <- factor(SD2$IRS, levels = c("preIRS", paste0("I-", 1:16)))
      
      if (s == "seasonal" & o == "closed") {
        ytitleC <- "black"
      } else if (s == "seasonal" & o == "regionally-open" & identical(nums, 237:245)) {
        ytitleC <- "black"
      } else if (s == "non-seasonal" & o == "regionally-open" & identical(nums, 201:210)) {
        ytitleC <- "black"
      } else {
        ytitleC <- "white"
      }
      if (s == "seasonal" & o == "semi-open" & identical(nums, 301:311)) {
        xtitleC <- "black"
      } else if (s == "non-seasonal" & o == "closed" & identical(nums, 101:111)) {
        xtitleC <- "black"
      } else if (s == "non-seasonal" & o == "regionally-open" & identical(nums, 237:246)) {
        xtitleC <- "black"
      } else {
        xtitleC <- "white"
      }
      p1 <- ggplot(SD2, aes(timePlot, mD, col = IRS, fill = IRS)) + 
        ...
        ggtitle(label) +
        xlab("Years Since IRS Starts \n(Sustained IRS)") + ylab("Number of Genes") +
        theme_bw() + theme(
      ...
      p2 <- ggplot(SD3, aes(x = meanCR, y = mD, col = IRS)) + 
        ...
        ggtitle(label) +
        xlab("Effective Contact Rate \n(per Host, per Year)") + ylab("Number of Genes") +
        theme_bw() +
        theme(
