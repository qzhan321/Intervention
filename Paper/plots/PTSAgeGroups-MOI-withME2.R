...
readDir <- paste0("/project2/pascualmm/QZ/PhD/projects/intervention/files", run, "/actualRuns/PTSAgeDiff2/")
...
dir.create(saveDir0)
saveDir1 <- paste0(saveDir0, "PTSAgeGroupsAndMOI2/PTS/")
dir.create(saveDir1)
state <- "withME"
  ...
  for (j in 1:length(openness)) {
    ...
    for (k in 1:length(numsPlot)) {
      ...
      for (m in 1:length(nums)) {
        ...
        load(file)
        if (m == 1) {
          PTSAllSub <- PTSAll %>% filter(num %in% nums[m], time == layers[1])
        ...
        PTSAllSub <- PTSAll %>% filter(num %in% nums[m], time == layers[2])
