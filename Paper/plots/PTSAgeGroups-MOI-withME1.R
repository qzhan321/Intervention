...
dir.create(saveDir0)
saveDir1 <- paste0(saveDir0, "PTSAgeGroupsAndMOI1/MOI/")
dir.create(saveDir1)
state <- "withME"
    ...
    for (k in 1:length(numsPlot)) {
      ...
      for (m in 1:length(nums)) {
        ...
        if (m == 1) {
          ...
          pts2x<-ggplot(MOIAllSub, 
                        aes(MOI, col=as.factor(time), fill =as.factor(time)))+
            geom_histogram(aes(group = time), bins = 40)+
            facet_grid(.~time)+
            ggtitle(paste0("Pre-IRS")) +
            theme_cowplot() +
            # scale_fill_manual(name = "time",values = colors, labels = c("Pre-IRS")) + ylab("Count") + xlab("MOI")+
            # scale_color_manual(name = "time", labels = c("Pre-IRS"), values = colors) +
            theme_classic() + theme(
            ...
            scale_fill_manual(name = "",values = c("dark orange")) +
            scale_color_manual(name = "",values = c("dark orange")) +
            scale_x_continuous(breaks = seq(1,21,5)) + guides(fill = "none", color = "none")
          ggsave(paste0(saveDir6, "MOI-", num, "-Pre-IRS.pdf"), pts2x, width = 5, height = 5)
        }
        ...
        pts2x<-ggplot(MOIAllSub, 
          ...
          theme_cowplot()+
          # scale_fill_manual(name = "time",values = colors[2], labels = c("IRS")) +
          ylab("Count")+xlab("MOI")+
          # scale_color_manual(name = "time", labels = c("IRS"), values = colors[2]) +
          theme_classic() + theme(
            plot.title = element_text(color="black", size=sizeV, hjust = 0.5),
            axis.title.x = element_text(color="black", size=sizeV),
            axis.title.y = element_text(color="black", size=sizeV),
            ...
            strip.text = element_blank()) +
          coord_cartesian(xlim = c(1,21)) +
          scale_fill_manual(name = "",values = c("dark blue")) +
          scale_color_manual(name = "",values = c("dark blue")) +
          scale_x_continuous(breaks = seq(1,21,5)) + guides(fill = "none", color = "none")
        print(pts2x)
        ggsave(paste0(saveDir6, "MOI-", num, "-IRS.pdf"), pts2x, width = 5, height = 5)
      }
    }
  }
}
...
saveDir0 <- paste0("/project2/pascualmm/QZ/PhD/projects/intervention/writings/simulation", run, "/plots/")
dir.create(saveDir0)
saveDir1 <- paste0(saveDir0, "PTSAgeGroupsAndMOI1/PTS/")
dir.create(saveDir1)
state <- "withME"
saveDir2 <- paste0(saveDir1, IRSType, "/")
dir.create(saveDir2)
    ...
    for (k in 1:length(numsPlot)) {
      ...
      for (m in 1:length(nums)) {
        ...
        if (m == 1) {
          ...
          pts2x<-ggplot(PTSAllSub, aes(x=PTS, fill = ageGroup, color = ageGroup))+
